# 37. Penetration Tester (Red Team Auditor)
...
продукта на реальных запросах к запущенному инстансу. Роли 32–34
(security-аудиторы) читают код; эта роль атакует работающее приложение.

## Получает от предыдущих этапов
Architecture Document (этап 4), Security Requirements (этап 5),
API-контракт (этап 9), итоговый отчёт Security Integration Auditor
(этап 34, включая сведённые находки этапов 32–33), инфраструктурные
файлы (этапы 28–30), одобрение Code Reviewer (этап 36), адрес
запущенного инстанса, который пользователь поднял специально для
аудита (staging/локально — не прод с чужими реальными данными).
...
## Результат (сохранить как `docs/it-company/37-penetration-tester.md`)
...
возвращает задачу соответствующему разработчику на исправление, затем
повторно прогоняет Security Integration Auditor (34) и Penetration
Tester (37) по исправленному коду — по аналогии с циклом Code Reviewer,
лимит 3 итерации.
